...
def generateSequence(length):
    sequence = [random.choice([0, 1, 2]) for _ in range(length)][0]
    return sequence
...
while True:
    ...
    start = 0
    while start < 3:
        # Counts three consecutive left clicks
    ...
    while alive:
        ...
        #Generates colours and displays them
        game.append(generateSequence(1))
        for light in game:
            display(light)
            sleep(2)
            display(-1)
        ...
        input = []
        #Lets user input their colours
        while len(input) < round:
            for j, pin in enumerate(button_pins):
                if machine.Pin(pin).value() == 0:
                    display(j)
                    input.append(j)
                    sleep(0.3)
